...
export interface ModelCapabilities {
  ...
  deprecated: boolean;       // Model is deprecated/outdated
  tier: "flagship" | "standard" | "fast" | "legacy";  // Performance tier
}
 ...
 */
const MODEL_CAPABILITIES: Record<LLMProvider, Record<string, ModelCapabilities>> = {
  openai: {
 ...
 */
export function getModelCapabilities(provider: LLMProvider, modelId: string): ModelCapabilities {
  ...
  const baseModelMatch = Object.keys(providerCapabilities).find((key) =>
    modelId.startsWith(key) || modelId.includes(key)
  );
 ...
 */
export function isDeprecated(provider: LLMProvider, modelId: string): boolean {
