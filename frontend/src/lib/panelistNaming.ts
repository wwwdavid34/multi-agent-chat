 ...
 * - "grok-2-1212" → "Grok 2"
 */
export function generateFriendlyModelName(provider: LLMProvider, modelId: string): string {
  if (!modelId) {
  ...
  if (provider === "openai") {
    ...
    if (modelLower.includes("o3-mini")) return "o3 Mini";
  ...
  if (provider === "gemini") {
    ...
    if (isFlash) return version ? `Gemini ${version} Flash` : "Gemini Flash";
    if (isPro) return version ? `Gemini ${version} Pro` : "Gemini Pro";
    return "Gemini";
  }
  ...
  if (provider === "grok") {
    ...
    if (versionMatch) return `Grok ${versionMatch[1]}`;
    return "Grok";
  }
...
export function isAutoGeneratedName(name: string, provider: LLMProvider, modelId: string): boolean {
  ...
  if (name === friendlyName) return true;
