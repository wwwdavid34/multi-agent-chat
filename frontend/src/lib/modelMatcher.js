 ...
 */
export function findBestModelMatch(targetModel, availableModels) {
    if (!targetModel || availableModels.length === 0) {
        return null;
    }
    const targetLower = targetModel.toLowerCase();
    // 1. Try exact match (case-insensitive)
    const exactMatch = availableModels.find((model) => model.id.toLowerCase() === targetLower);
    if (exactMatch) {
        return exactMatch;
    }
    // 2. Try prefix match - target is prefix of available model
    // e.g., "gemini-1.5-pro" matches "gemini-1.5-pro-latest"
    const prefixMatch = availableModels.find((model) => model.id.toLowerCase().startsWith(targetLower));
    if (prefixMatch) {
        return prefixMatch;
    }
    // 3. Try contains match - available model contains target
    // e.g., "claude-sonnet" matches "claude-3-5-sonnet-20241022"
    const containsMatch = availableModels.find((model) => model.id.toLowerCase().includes(targetLower));
    if (containsMatch) {
        return containsMatch;
    }
    // 4. Try reverse contains - target contains available model
    // e.g., "gemini-1.5-pro-002" in target matches "gemini-1.5-pro"
    const reverseContainsMatch = availableModels.find((model) => targetLower.includes(model.id.toLowerCase()));
 ...
 */
function extractBaseModelName(modelId) {
    return modelId
