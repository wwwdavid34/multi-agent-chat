...
import { askPanel, askPanelStream, fetchStorageInfo, generateTitle, getAllConversations, saveConversationMessage, deleteConversationMessage, checkProvidersQuota, type MessageEntry as ApiMessageEntry, type QuotaStatus } from "./api";
...
const MessageBubble = memo(function MessageBubble({
  ...
  userVotes?: Record<string, "compelling" | "weak">;
...
export default function App() {
  ...
  // Initialize with empty conversations - will be loaded from server for authenticated users
  ...
  // Wrap setLoading to log changes
  const setLoading = useCallback((value: boolean) => {
    console.log("[DEBUG] setLoading called:", { previous: loadingInternal, new: value, stack: new Error().stack?.split('\n').slice(1, 4).join(' -> ') });
  ...
  const [activeEntryId, setActiveEntryIdInternal] = useState<string | null>(null);
  // Wrap setActiveEntryId to log changes
  const setActiveEntryId = useCallback((value: string | null) => {
    console.log("[DEBUG] setActiveEntryId called:", { previous: activeEntryId, new: value, stack: new Error().stack?.split('\n').slice(1, 4).join(' -> ') });
    setActiveEntryIdInternal(value);
  }, [activeEntryId]);
  ...
  const [abortControllerInternal, setAbortControllerInternal] = useState<AbortController | null>(null);
  // Wrap setAbortController to log changes
  const setAbortController = useCallback((value: AbortController | null) => {
    console.log("[DEBUG] setAbortController called:", { previous: abortControllerInternal ? "exists" : "null", new: value ? "exists" : "null", stack: new Error().stack?.split('\n').slice(1, 4).join(' -> ') });
    setAbortControllerInternal(value);
  }, [abortControllerInternal]);
  const abortController = abortControllerInternal;
  ...
  const { isAuthenticated, isLoading: authLoading, accessToken, migrateThreads, fetchThreads, systemKeyStatus } = useAuth();
  ...
  const activeEntry = useMemo(() => {
    if (!activeEntryId) {
      console.log("[DEBUG] activeEntry: activeEntryId is null");
      return null;
    }
    const entry = messages.find((entry) => entry.id === activeEntryId) ?? null;
    console.log("[DEBUG] activeEntry computed:", {
      activeEntryId,
      found: !!entry,
      debate_paused: entry?.debate_paused,
  ...
  // But don't clear if there's an active debate (to avoid race conditions)
  useEffect(() => {
    if (!authLoading && !isAuthenticated) {
      // Don't clear during an active debate - could cause race conditions
      if (activeEntryId) {
        console.log("[DEBUG] Guest cleanup skipped - active debate in progress", { activeEntryId });
        return;
      }
      console.log("[DEBUG] Guest mode: clearing conversations");
  ...
  }, [isAuthenticated, authLoading, activeEntryId]);
  ...
  useEffect(() => {
    const restoreData = async () => {
      ...
      if (!authLoading && isAuthenticated && accessToken && !conversationsLoaded) {
        console.log("[Data Restore] Starting data restoration from server...");
        try {
          // Fetch threads and conversations in parallel
          const [serverThreads, serverConversations] = await Promise.all([
            fetchThreads(),
            getAllConversations(),
          ]);

          console.log(`[Data Restore] Server returned ${serverThreads.length} threads`);
          console.log(`[Data Restore] Server returned conversations for ${Object.keys(serverConversations.conversations).length} threads`);
          ...
          // Restore conversations - convert from API format to local format
          const normalized: ConversationMap = { [DEFAULT_THREAD_ID]: [] };
          Object.entries(serverConversations.conversations).forEach(([tid, messages]) => {
            normalized[tid] = (messages ?? []).map((msg: ApiMessageEntry) => ({
              id: msg.id,
              ...
              debate_mode: msg.debate_mode as DebateMode | undefined,
              ...
              tagged_panelists: msg.tagged_panelists ?? undefined,
            }));
          });
          setConversations(normalized);
          setConversationsLoaded(true);

          console.log(`[Data Restore] Successfully restored ${serverThreads.length} threads and conversations`);
        } catch (err) {
          console.error("[Data Restore] Failed to restore data:", err);
    ...
    restoreData();
  }, [isAuthenticated, authLoading, accessToken, fetchThreads, conversationsLoaded]);
  ...
  // Note: System keys are no longer fetched from backend (BYOK enforcement)
  // Authenticated users' stored keys are loaded via AuthContext
  useEffect(() => {
    const storedKeys = parseJSON<ProviderKeyMap>(localStorage.getItem("providerKeys"), {});
    const keyProviders = Object.keys(storedKeys).filter(k => storedKeys[k as LLMProvider]?.trim());
    console.log('[API Keys] Loaded from localStorage:', keyProviders.join(', ') || 'none');
  ...
  // Helper to save a message to the server (for authenticated users)
  const saveMessageToServer = useCallback(
    async (tid: string, entry: MessageEntry) => {
      if (!isAuthenticated || !accessToken) {
        return; // Skip for guests
      }
      try {
        const apiEntry: ApiMessageEntry = {
          id: entry.id,
          ...
          panelists: entry.panelists,
          debate_history: entry.debate_history || null,
          debate_mode: entry.debate_mode || null,
          max_debate_rounds: entry.max_debate_rounds ?? null,
          debate_paused: entry.debate_paused,
          stopped: entry.stopped,
          usage: entry.usage || null,
          tagged_panelists: entry.tagged_panelists || null,
        };
        await saveConversationMessage(tid, apiEntry);
        console.log(`[Persistence] Saved message ${entry.id} to server`);
      } catch (err) {
        console.error(`[Persistence] Failed to save message ${entry.id}:`, err);
  ...
  const handleSend = useCallback(
    async ({
      ...
      // Create abort controller FIRST so stop button is available immediately
      ...
      try {
        await askPanelStream(
          ...
          },
          {
            ...
            onDebatePaused: (result) => {
              console.log("[DEBUG] onDebatePaused called", { entryId, threadId, result });
              setEntryStatus(entryId, "Paused for review");
              // Debate paused for user review - save to server
              setConversations((prev) => {
                const entry = prev[threadId]?.find((e) => e.id === entryId);
                console.log("[DEBUG] Entry before update:", { entryId, entry, debate_mode: entry?.debate_mode });
                ...
                const finalEntry = updatedConversations[threadId]?.find((e) => e.id === entryId);
                if (finalEntry) {
                  saveMessageToServer(threadId, finalEntry);
                }
            ...
            onResult: (result) => {
              ...
              // Update the entry with the actual response and save to server
              setConversations((prev) => {
                const updatedConversations = {
                  ...prev,
                  [threadId]: prev[threadId]?.map((entry) =>
        ...
        if (err instanceof Error && err.name === 'AbortError') {
          ...
          // Keep the entry but mark it as stopped - save to server
          setConversations((prev) => {
            const updatedConversations = {
              ...prev,
              [threadId]: prev[threadId]?.map((entry) =>
    ...
    [clearEntryStatus, loading, preparedPanelists, sanitizedProviderKeys, setEntryStatus, threadId, saveMessageToServer, isAuthenticated, systemKeyStatus, panelists, enabledPanelists, providerKeys]
  );
  ...
  const handleContinueDebate = useCallback(
      ...
      try {
        await askPanelStream(
          ...
          },
          {
            ...
            onDebatePaused: (result) => {
              setEntryStatus(entryId, "Paused for review");
              // Still paused - waiting for next round - save to server
              setConversations((prev) => {
                const updatedConversations = {
                  ...prev,
                  [threadId]: prev[threadId]?.map((e) =>
            ...
            onResult: (result) => {
              clearEntryStatus(entryId);
              // Debate complete - got final summary - save to server
              setConversations((prev) => {
                const updatedConversations = {
                  ...prev,
                  [threadId]: prev[threadId]?.map((e) =>
    ...
    [clearEntryStatus, conversations, preparedPanelists, sanitizedProviderKeys, setEntryStatus, threadId, saveMessageToServer]
  );

  // Track user votes for visual feedback (key: `${entryId}-${panelistName}-${roundNumber}`)
  const [userVotes, setUserVotes] = useState<Record<string, "compelling" | "weak">>({});

  const handleVote = useCallback(
    (entryId: string, panelistName: string, roundNumber: number, voteType: "compelling" | "weak") => {
      const voteKey = `${entryId}-${panelistName}-${roundNumber}`;
      ...
      if (existingVote === voteType) {
        ...
        console.log(`[Vote] Removed ${voteType} vote for ${panelistName} in round ${roundNumber + 1}`);
      } else {
        // New vote or changing vote
        setUserVotes((prev) => ({ ...prev, [voteKey]: voteType }));
        console.log(`[Vote] ${existingVote ? `Changed to ${voteType}` : voteType} vote for ${panelistName} in round ${roundNumber + 1}`);
      }
  ...
  const handleFetchProviderModels = useCallback(
    async (provider: LLMProvider) => {
      ...
      // Check if system key is available for this provider
      const providerToStatusKey: Record<string, "openai" | "anthropic" | "google" | "xai"> = {
        openai: "openai",
        gemini: "google",
        claude: "anthropic",
        grok: "xai",
      };
      const statusKey = providerToStatusKey[provider];
      const hasSystemKey = statusKey && systemKeyStatus ? systemKeyStatus[statusKey] : false;
      ...
      try {
        // Pass empty string if using system key - backend will use system key for allowlisted users
        const models = await fetchModelsForProvider(provider, apiKey, accessToken || undefined);
    ...
    [providerKeys, systemKeyStatus, accessToken]
  );
  ...
  const handlePanelistChange = useCallback(
      ...
      setPanelists((prev) =>
        prev.map((panelist) => {
          ...
          if (providerChanged) {
            ...
            // Auto-fetch models if provider changed and we have an API key (local or system)
            const hasLocalKey = providerKeys[nextProvider]?.trim();
            const hasModels = (providerModels[nextProvider]?.length ?? 0) > 0;

            // Check for system key access
            const providerToStatusKey: Record<string, "openai" | "anthropic" | "google" | "xai"> = {
              openai: "openai",
              gemini: "google",
              claude: "anthropic",
            ...
            const statusKey = providerToStatusKey[nextProvider];
            const hasSystemKey = statusKey && systemKeyStatus ? systemKeyStatus[statusKey] : false;

            if ((hasLocalKey || hasSystemKey) && !hasModels) {
    ...
    [providerModels, providerKeys, handleFetchProviderModels, systemKeyStatus]
  );
  ...
  const deleteMessage = useCallback((index: number) => {
    // Get message ID before deleting from state
    const messageToDelete = conversations[threadId]?.[index];
    ...
    if (messageId && isAuthenticated && accessToken) {
      deleteConversationMessage(threadId, messageId).catch((err) => {
        console.error("[Persistence] Failed to delete message from server:", err);
      });
    }
  }, [threadId, conversations, isAuthenticated, accessToken]);
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden bg-background">
          ...
          {threads.map((id) => (
            <li key={id} className="group relative">
              <div className="flex items-center relative">
                ...
                {editingThreadId !== id && (
                <div className="absolute right-1 top-1/2 -translate-y-1/2 z-10" data-thread-menu>
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  <AnimatePresence>
                    {threadMenuOpen === id && (
                      <motion.div
                        ...
                        className="absolute right-0 top-full mt-1 z-[100] min-w-[140px] rounded-lg border border-border shadow-xl py-1 pointer-events-auto bg-neutral-900 dark:bg-neutral-900 text-neutral-100"
                      ...
                      >
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-3 py-2 text-[13px] text-neutral-100 hover:bg-neutral-700 transition-colors text-left"
                        >
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-3 py-2 text-[13px] text-red-400 hover:bg-red-900/30 transition-colors text-left"
                        >
                          <svg viewBox="0 0 24 24" className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2">
                            <path d="M3 6h18" />
      ...
      <main className="flex-1 overflow-hidden bg-transparent flex flex-col">
        ...
        <section className={`flex flex-1 min-h-0 flex-col relative ${messages.length === 0 ? 'items-center justify-center' : ''}`}>
              ...
              <div className={`flex flex-col gap-10 py-10 pb-16 mx-auto w-full max-w-3xl px-4 sm:px-6 ${messages.length === 0 ? 'hidden' : ''}`}>
                {messages.map((entry, index) => (
                  <MessageBubble
                    ...
                    onVote={(panelistName, roundNumber, voteType) => handleVote(entry.id, panelistName, roundNumber, voteType)}
                    userVotes={Object.fromEntries(
                      Object.entries(userVotes)
                        .filter(([key]) => key.startsWith(`${entry.id}-`))
                        .map(([key, value]) => [key.replace(`${entry.id}-`, ""), value])
                    )}
            ...
            <div className="pt-4 pb-4 relative z-10 mx-auto w-full max-w-3xl px-4 sm:px-6">
                ...
                <div className="mb-2 rounded-xl border border-border/50 bg-background/90 backdrop-blur-sm px-3 py-2 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {/* Always-visible stop button in the status bar */}
                      <button
                        onClick={stopGeneration}
                        className="ml-2 px-3 py-1 rounded-lg bg-destructive/10 hover:bg-destructive/20 text-destructive text-xs font-medium transition-colors border border-destructive/30 flex items-center gap-1.5"
              ...
              {/* Debug: Log activeEntry state */}
              {(() => {
                console.log("[DEBUG] activeEntry for continue dialog:", {
                  activeEntryId,
                  activeEntry: activeEntry ? {
                    id: activeEntry.id,
                    debate_paused: activeEntry.debate_paused,
                    debate_mode: activeEntry.debate_mode,
                  } : null,
                  shouldShowSupervisedDialog: activeEntry?.debate_paused && activeEntry?.debate_mode === "supervised",
                });
                return null;
              })()}
      ...
      <PanelConfigurator
        ...
        systemKeyStatus={systemKeyStatus}
      />
